  ...
  <LandingContainer>
    <header class="flex flex-col lg:flex-row justify-between items-center my-5">
      <div class="flex w-full lg:w-auto items-center justify-between">
        <NuxtLink href="/" class="text-lg flex items-center">
          <img class="h-10 w-10 rounded-full me-1" src="@/assets/img/logo.jpg" alt="Your Company" />
          <span class="font-bold text-slate-800">Code</span><span class="text-slate-500">Devper</span>
        </NuxtLink>
      ...
      <nav class="w-full lg:w-auto mt-2 lg:flex lg:mt-0" :class="{ block: open, hidden: !open }">
        <ul class="flex flex-col lg:flex-row lg:gap-3">
          <li v-for="item of menuitems">
            <NuxtLink :href="item.path" class="flex lg:px-3 py-2 text-gray-600 hover:text-gray-900">
              {{ item.title }}
            </NuxtLink>
          </li>
        </ul>
